...
public class ClasseDAO implements IDAO{
    ...
    public Classe readFromIdStudente(int idStudente){
        String query = "select c.* from classi c join studenti s on s.id_classe = c.id where s.id = ?";
        PreparedStatement ps = null;
        ResultSet rs = null;
        Classe c = null;
        try{
            ...
            if(rs.next()){
                ...
                // context.getBean(Snack.class, rs.getInt(1), rs.getString(2), rs.getDouble(3));
                c = context.getBean(Classe.class, params);
            }
